!==================================================================================================
! This is a stub/refactor for the 'contituents_mod' to enable compatibility of 'mo_gocart'
! in EAMxx. 
!
! Refactor notes:
!   - PCSNT (total number of advected species) is not known at compile-time of this translation 
!     unit. We must have 'pcnst >= gas_pcnst' for the 3D array of chemical tracers but other
!     tracers may not be identical between EAM and EAMxx and generally should be passed by 
!     argument directly rather than through the agglomorated tracer (q) array.
!
...
! Public data
  ...
  !-- modified by D. Fust 2025-08-24 
  !-- Refactor: The example case uses PCNST=101 though it is not particularly safe
  !             to set parameters with this design pattern in Fortran as it is case agnostic
  !             and is commonly written in all caps. It may be possible to shrink 
  !             `pcnst` to `gas_pcnst` as only the GOCART chemical tracers should 
  !             need the indexing and labeling infrastructure enabled by this module.
  !integer, parameter, public :: pcnst  = gas_pcnst      ! number of advected constituents
  integer, parameter, public :: pcnst  = 101      ! According to example case
