...
! module mo_sim_dat
!
! This module is a minor refactor of the EAM 'mo_sim_dat' module from 
! ${EAM}/src/chemistry/pp_chemuci_linozv3_mam5_vbs
!
! This module sets simulation data required to run 'mo_gocart' and its dependencies. 
!
! NOTE: 'cam_abortutils' and 'cam_logfile' appear to be safe for direct use in EAMxx.
!
...
module mo_sim_dat
  ...
  contains
    ...
    !
    ! Sets simulation data for the GOCART model
    !----------------------------------------------------------------------------------------------
    subroutine set_sim_dat()
      ...
      !-- Initialize chem_mods variables --
      clscnt(:) = (/     36,     0,     0,    37,     0 /)
      cls_rxt_cnt(:,1) = (/      9,    14,     0,    36 /)
      cls_rxt_cnt(:,4) = (/      2,    50,    50,    37 /)

      !-- Initialize mo_tracname tracer names --
