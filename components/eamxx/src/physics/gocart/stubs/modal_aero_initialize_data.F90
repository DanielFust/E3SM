!==================================================================================================
! This module has been heavily stubbed to remove useage of the `time_manager`, `physics_buffer`, 
! `phys_control`, and many initialization routines for modal aerosol models for wet and dry
! deposition processes, and sea salt models. These may be required for a full bridge to 
! `mo_gocart`, but are themselves difficult to bridge due to their usage of the physics buffer and
! their reliance on unknown user settings and preprocessor directives.
!
! At present, this module is only factored to support initialization of data required to run
! the cloud aerosol chemistry process in `mo_setsox.F90`. 
!==================================================================================================
...
!  end subroutine modal_aero_register
  ...
  subroutine modal_aero_initialize()!pbuf2d, imozart, species_class) 
       ...
       !use modal_aero_calcsize,   only: modal_aero_calcsize_init !<-- transfer between modes?? 
       !use modal_aero_coag,       only: modal_aero_coag_init     !<-- Coagulation of aerosols
...
!          end if
           imozart=1 !<-- currently we are only using `gas_pcnst` tracers in the array
          do i = imozart, pcnst
...
!       end if ! use_MMF
       ...
       !-----------------------------------------------------------
       ! EAMxx side should take care of initializations
       !  Daniel Fust (Sept. 2025)
       !-----------------------------------------------------------
       ...
       !-----------------------------------------------------------
       ! modal_aero_convproc mostly deals with convection 
       ! processes and some wet removal processes in clouds. It
       ! seems safe to assume that this is outside the scope of 
       ! the GOCART atmosphere process.
       ! DJF 9/4/2025 
       !-----------------------------------------------------------
