!==================================================================================================
! This stubbed version of the `sox_cldaero_mod` module from 
! ${EAM_ROOT}/src/chemistry/modal_aero/sox_cldaero_mod.F90 has removed references to
! the following:
!   - cam_history: this module controls how simulation history is recorded and should not be
!                  available nor required in EAMxx
!   - phys_control: this module is only used for history, which is not being bridged
!==================================================================================================
!----------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------
  function sox_cldaero_create_obj(cldfrc, qcw, lwc, cfact, ncol, loffset) result( conc_obj )
    ...
    !write(*,*) "In sox_cldaero_create_obj..."

    mode7 = ntot_amode == 7
    mode9 = ntot_amode == 9

    conc_obj => cldaero_allocate()
    !write(*,*) "sox_cldaero_create_obj: returned from cldaero_allocate..."
    ...
    do k = 1,pver
       ...
       enddo
    enddo
    !write(*,*) "sox_cldaero_create_obj: completed lwc loop..."

    conc_obj%no3c(:,:) = 0._r8

    if (mode7 .or. mode9) then
      !write(*,*) "sox_cldaero_create_obj: mode7 or mode9..."
...
!put ifdef here so ifort will compile 
       ...
       !write(*,*) "sox_cldaero_create_obj: 3 or 4 modes..."
       !note: here is 4 and 3 mode cases,the 4th-mode here has no so4     
       id_so4_1a = lptr_so4_cw_amode(1) - loffset
       id_so4_2a = lptr_so4_cw_amode(2) - loffset
       id_so4_3a = lptr_so4_cw_amode(3) - loffset
       !write(*,*) "sox_cldaero_create_obj:"
       !write(*,*) "    id_so4_1a=",id_so4_1a
       !write(*,*) "    id_so4_2a=",id_so4_2a
       !write(*,*) "    id_so4_3a=",id_so4_3a
       !write(*,*) "    size: qcw=",size(qcw,dim=1),size(qcw,dim=2),size(qcw,dim=3)
       conc_obj%so4c(:ncol,:) &
            = qcw(:,:,id_so4_1a) &
            + qcw(:,:,id_so4_2a) &
            + qcw(:,:,id_so4_3a)
       ...
       !write(*,*) "sox_cldaero_create_obj: actually 5 modes..."
       id_so4_1a = lptr_so4_cw_amode(1) - loffset
       id_so4_2a = lptr_so4_cw_amode(2) - loffset
       id_so4_3a = lptr_so4_cw_amode(3) - loffset
       id_so4_4a = lptr_so4_cw_amode(5) - loffset
