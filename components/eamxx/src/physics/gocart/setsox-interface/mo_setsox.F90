!==================================================================================================
! The `mo_setsox` module has been stubbed from ${EAM_ROOT}/src/chemistry/aerosol/mo_setsox.F90
! 
! NOTES:
!   - Removed references to the following modules:
!       - cam_history
!       - phys_control
!     These modules were used to write history data, but are not being bridged to EAMxx
!
!   - Removed `invariants` array from `SETSOX` subroutine.
!     `invariants` appears to refer to species that are prescribed rather than advected. This was
!     largely done due to technical reasons, as the `invariants` array retrives its values from
!     the physics buffer and it is not presently clear where this information is ultimately set.
!       This may still be in line with the provided example case, which uses the 
!     pp_chemuci_linozv3_mam5 5 mode aerosol model. In the `chem_mech.doc` document, the 
!     invariants are specified as:
!       (  1)  M               
!       (  2)  N2              
!       (  3)  O2              
!       (  4)  H2O             
...
!       (  8)  prsd_NO3        
!       (  9)  prsd_OH   
!     This may indicate the O3 is intended to be prescribed rather than advected. However, even
!     if this is the case, it would still be easier to include all of the species in the 'qin' 
!     array and prescribe a value to the invariant species prior to calling `SETSOX`. This is 
!     because many of the procedures in `mo_gocart` expect an array with 
!     `dimension(pcol,pver,gas_pcnst)`, where `gas_pcnst` is 73 which includes all 36 "Explicit" 
!     and 37 "Implicit" species. Species that are not advected can be added as 'Fields' rather
!     than 'tracers' in the EAMxx AtmosphereProcess if needed. An equivalent solution would be to
!     pass the `qin` array as the argument for both the `qin` dummy argument and `invariants` 
!     dummy argument as they appear to both share the `dimension(pcol,pver,gas_pcnst)` with 
!     consistent species indexing. The present refactor has elected to remove the `invariants`
!     argument altogether for simplicity and to avoid potential confusion.
!
...
module MO_SETSOX
  ...
  !-- The following were local variables from the SETSOX
  !   subroutine, which are retrieved from the run settings
  !   with `phys_getopts`. In this refactor, they are module
  !   variables set by argument in `sox_inti`
  logical :: use_MMF  = .false.
  logical :: use_ECPP = .false.

contains

  !-----------------------------------------------------------------------      
  ! Refactor notes:
  !   Because `phys_control` is no longer used due to compatibility 
  !   nightmare, `phys_opts` can no longer be used to retrieve settings 
  !   during runtime. As such, the values of `modal_aero` must be passed 
  !   by argument.
  ! 
  ! D. Fust (Aug 2025)
  !-----------------------------------------------------------------------      
  subroutine sox_inti(use_modal_aerosols, MMF, ECPP)
    !-----------------------------------------------------------------------      
    ...
    implicit none
    !---------------- Dummy Arguments ------------------
    logical, intent(in), optional :: use_modal_aerosols
    logical, intent(in), optional :: MMF                ! 
    logical, intent(in), optional :: ECPP               ! explicit cloud parameterized pollutants
    !---------------------------------------------------
    ...
    !-- Refactored to set via argument (D. Fust) --
    if (present(use_modal_aerosols)) modal_aerosols = use_modal_aerosols
    if (present(MMF))                use_MMF        = MMF
    if (present(ECPP))               use_ECPP       = ECPP
...
!-----------------------------------------------------------------------      
! NOTE: `invariants` has been removed
!-----------------------------------------------------------------------      
  subroutine SETSOX( &
       ncol,   &
       lchnk,  &
    ...
    !-- refactored as module variables (D. Fust) --
    !logical :: use_MMF, use_ECPP
    write(*,*) "In SETSOX..."

    !-- xhno3 was already an automatic array. Not sure how these lines ever compiled (D. Fust) --
    !real(r8), allocatable :: xhno3(:,:)
    !allocate(xnho3(ncol,pver))
    ...
    xnh4c => cldconc%nh4c
    xno3c => cldconc%no3c
    write(*,*) "SETSOX: sox_cldaero object created..."
...
!       endif
    ...
    write(*,*) "SETSOX: Temperature dependent Henry constants..."
    ...
    write(*,*) "SETSOX: Now use the actual PH..."
    !==============================================================
